...
public final class HttpLoggingInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if(request.url().toString().contains("log")){
            return chain.proceed(request);
        }
...
//                    LogUtils.i("Content-Type: " + requestBody.contentType());
                }
                if (requestBody.contentLength() != -1) {
//                    LogUtils.i("Content-Length: " + requestBody.contentLength());
                }
            }
...
//        LogUtils.i("--> END " + request.method() + " (encoded body omitted)");
        ...
        String logTemp = "<-- " + response.code() + ' ' + response.message() + ' '
