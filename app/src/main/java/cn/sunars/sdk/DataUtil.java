...
public class DataUtil {
	...
	public static short Crc16(byte[] data, int len)// uchar *ptr, uchar len
	{
		...
		while (len-- != 0) {
			...
			Crc ^= crc_ta[(da ^ (data[i] >>> 4))
					& 0xf]; /* CRC的高4位和本字节的前半字节相加后查表计算CRC， 然后加上上一次CRC的余数 */
	...
	public static  int getUnsignedShort(short data) { // 将data字节型数据转换为0~255 (0xFF
												// 即BYTE)。
		return data & 0x0FFFF;
	}

	public static void printDataBuf(byte[] buf, String flag) {
	...
	public static boolean checkPack(byte[] buf) {
		if (buf.length > 4 && getUnsignedByte(buf[0]) == 0xF5 && getUnsignedByte(buf[1]) == 0xAA
				&& getUnsignedByte(buf[2]) == 0xAA) {
			return true;
		}
		return false;
	}
