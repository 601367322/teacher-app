package com.prance.sharedpreferences;
...
public class SharedDataUtil {
    ...
    private static Map<String, SharedDataUtil> mSharedDataUtilMap = new HashMap<>();
    ...
    public SharedDataUtil(Context context, String dbName) {
        if (sharedDataSqLiteHelper == null || sharedDataSqLiteHelper.isClosed()) {
            this.sharedDataSqLiteHelper = new SharedDataSqLiteHelper(context, dbName);
        }
    }
    ...
    public static synchronized SharedDataUtil getInstance(Context context) {
        ...
        if (!mSharedDataUtilMap.containsKey(SharedDataSqLiteHelper.DB_NAME)) {
            mSharedDataUtilMap.put(SharedDataSqLiteHelper.DB_NAME, new SharedDataUtil(context));
        }

        return mSharedDataUtilMap.get(SharedDataSqLiteHelper.DB_NAME);
    }

    public static synchronized SharedDataUtil getInstance(Context context, String dbName) {
        if (context == null) {
            return null;
        }

        if (!mSharedDataUtilMap.containsKey(dbName)) {
            mSharedDataUtilMap.put(dbName, new SharedDataUtil(context, dbName));
        }

        return mSharedDataUtilMap.get(dbName);
    }
    ...
    public Map<String, Object> getAll() {
        ...
        List<SharedData> list = sharedDataSqLiteHelper.getAllData();
        for (int i = 0; i < list.size(); i++) {
            Object obj = null;
            switch (list.get(i).getDataType()) {
                case INT:
                    obj = list.get(i).getmInt();
                    break;
                case STRING:
                    obj = list.get(i).getmStr();
                    break;
                case BOOLEAN:
                    obj = list.get(i).ismBoolean();
                    break;
                case DATA:
                    obj = list.get(i).getmDate();
                    break;
                case LONG:
                    obj = list.get(i).getmLong();
                    break;
                case FLOAT:
                    obj = list.get(i).getmFloat();
                    break;
            }
            map.put(list.get(i).getKey(), obj);
        }
        return map;
    }
