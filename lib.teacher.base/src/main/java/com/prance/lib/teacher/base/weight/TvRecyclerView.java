                ...
                switch (event.getKeyCode()) {
                    case KeyEvent.KEYCODE_DPAD_RIGHT:
                        ...
                        if (rightView != null) {
                            ...
                            int position = getLayoutManager().getPosition(focusView);
                            int allChild = getLayoutManager().getItemCount();
                            if (isRightEdge(position) && position < allChild - 1) {
                                View nextView = getLayoutManager().findViewByPosition(position + 1);
                    ...
                    case KeyEvent.KEYCODE_DPAD_LEFT:
                        ...
                        if (leftView != null) {
                            ...
                            int position = getLayoutManager().getPosition(focusView);
                            if (isLeftEdge(position) && position > 0) {
                                View nextView = getLayoutManager().findViewByPosition(position - 1);
                                nextView.requestFocus();
                                requestPreLineFocus(nextView);
                                return true;
                            } else {
                                return false;
                            }
...
//                            int position = getLayoutManager().getPosition(focusView);
//                            if (!isBottomEdge(position)) {
//                                int allChild = getLayoutManager().getItemCount();
//                                int spanCount = 1;
//                                if (getLayoutManager() instanceof GridLayoutManager) {
//                                    spanCount = ((GridLayoutManager) getLayoutManager()).getSpanCount();
//                                }
//                                int nextPosition = position + spanCount;
//                                if (nextPosition >= allChild - 1) {
//                                    nextPosition = allChild - 1;
//                                }
...
//                            }
    ...
    public void requestNextLineFocus(View view) {
        int downOffset = view.getTop() + view.getHeight() / 2 - getHeight() / 2;
        this.smoothScrollBy(0, downOffset);
    }

    public void requestPreLineFocus(View view) {
        int upOffset = getHeight() / 2 - (view.getBottom() - view.getHeight() / 2);
        this.smoothScrollBy(0, -upOffset);
    }
